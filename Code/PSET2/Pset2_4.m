...
%% pseudorandom samples from a Rayleigh Distribution
N = 1000;   % N number of samples per set
S = 1000;   % S number of sets
b = 5;  % parameter of rayleigh distribution
...
for i = 1
    ...
    % set the location of the FALSE pdf
    x = x_edges(1):0.001:x_edges(end);
    GaussPdf = pdf('norm',x,b*sqrt(pi/2),sqrt(b^2*((4-pi)/2)));
    
    figure(5);
    title('Samples from Rayleigh Distribution compared to Gaussian pdf')
    grid on; hold on;
